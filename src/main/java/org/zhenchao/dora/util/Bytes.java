...
import org.zhenchao.dora.unsafe.UnsafeAccessor;
import org.zhenchao.dora.unsafe.UnsafeChecker;
import org.zhenchao.dora.util.Bytes.LexicographicalComparerHolder.UnsafeComparer;
...
public class Bytes {
    ...
    private static final boolean UNSAFE_UNALIGNED = UnsafeChecker.unaligned();
    ...
    public static byte[] toBytes(String s) {
        return null == s ? null : s.getBytes(UTF8_CHARSET);
    }
    ...
    public static int putLongUnsafe(byte[] bytes, int offset, long val) {
        ...
        UnsafeComparer.theUnsafe.putLong(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET, val);
        return offset + SIZEOF_LONG;
    }
     ...
     * Big-endian, Same as what DataOutputStream.writeInt does.
     *
     * @param val value
     * @return the byte array
     */
    ...
    static class LexicographicalComparerHolder {
        ...
        enum UnsafeComparer implements Comparer<byte[]> {
            ...
            static {
                if (UNSAFE_UNALIGNED) {
                    theUnsafe = UnsafeAccessor.theUnsafe;
                } else {
     ...
     */
    public static int indexOf(byte[] array, byte[] target) {
        if (null == array || null == target) {
            throw new IllegalArgumentException("input args can't be null");
        }
        if (target.length == 0) {
            return 0;
        }
    ...
    public static void zero(byte[] b, int offset, int length) {
        if (length <= 0 || offset >= b.length || (offset + length) >= b.length) {
            throw new IllegalArgumentException(
                    "illegal args, offset:" + offset + ", length:" + length + ", byte length:" + b.length);
        }
    ...
    public static String toHex(byte[] b, int offset, int length) {
        if (length > Integer.MAX_VALUE / 2) {
            throw new IllegalArgumentException("length must be less(or equal) than half of " + Integer.MAX_VALUE);
        }
    ...
    /*public static boolean isBoolean(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_BOOLEAN) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toBoolean(bytes)));
    }

    public static boolean isShort(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_SHORT) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toShort(bytes)));
    }

    public static boolean isChar(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_CHAR) {
            return false;
        }
        return !Bytes.equals(bytes, toBytes(toShort(bytes)));
    }*/

    /*public static boolean isInteger(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_INT) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toInt(bytes)));
    }*/

    /*public static boolean isFloat(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_FLOAT) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toFloat(bytes)));
    }

    public static boolean isLong(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_LONG) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toLong(bytes)));
    }

    public static boolean isDouble(byte[] bytes) {
        if (null == bytes || bytes.length != SIZEOF_DOUBLE) {
            return false;
        }
        return Bytes.equals(bytes, toBytes(toDouble(bytes)));
    }*/
